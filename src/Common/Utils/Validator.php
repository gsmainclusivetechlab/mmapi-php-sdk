...
class Validator
{
    ...
    public function validateField(
        ...
        if (is_callable([$this->objToValidate, $method])) {
            ...
            switch ($type) {
                case self::MANDATORY:
                    if (empty($value)) {
                        $this->addError(
                            $fieldName,
                            $this->defaultErrorMsg[self::MANDATORY]
                        );
                    }
                    break;
                case self::VALID_AMOUNT:
                    ...
                    if (!preg_match($pattern, $value) && !empty($value)) {
                        $this->addError(
                            $fieldName,
                            $this->defaultErrorMsg[self::VALID_AMOUNT]
                        );
                    }
                    break;
                default:
                    throw new Exception(
                        'Validator: ' + $type + ' is not a valid type.'
                    );
            }
        } else {
            throw new Exception(
                'Validator: ' + $fieldName + ' is not a valid field.'
            );
        }
    }
    ...
    public function getValidationErrors()
    {
        if (empty($this->validationErrors)) {
            ...
            foreach (
                ...
                as $validationErrorKey => $validationErrorValue
            ) {
